export const quizData = [
  {
    id: 0,
    question: ` Donde se desarrolla zombies Nazis ?`,
    options1: [`Belgica`, `Alemania`, `francia`],
    answer: `Alemania`  },
  {
    id: 1,
    question: `Que actor hacia de si mismo en bienvenidos a Zombieland?`,
    options1: [`Bill Murray`, `Woody Harrelson`, `Elon Musk`],
    answer: `Bill Murray`
  },
  {
    id: 3,
    question: `De que año es Zombie el regreso de los Muertos Vivientes?`,
    options1: [`1973`, `1978`, `1983`],
    answer: ` 1978`
  },
  {
    id: 4,
    question: `Que sucede con el bebe de Rosemary?`,
    options1: [`Es el hijo del diablo`, `Lo secuestran despues de nacer `, `Lo aborta inesperadamente`],
    answer: `Es el hijo del diablo`
  }
];